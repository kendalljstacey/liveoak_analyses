...
adults
ggplot(speciescountdate %>% filter(Species=="EQ"|Species=="NV"|Species=="ES"))+
  geom_col(aes(x=Date, y=Count, fill=Species))
ggplot(speciescountdate %>% filter(Species=="Adults total"|Species=="Nymphs"))+
  geom_point(aes(x=Date, y=Count, color=Species, shape=Species), size=2)
ggplot(speciescountdate %>% filter(Species=="Adults total"& Location!="Sorghum"))+
  geom_point(aes(x=Date, y=Count, color=Treatment, shape=Treatment), size=3)
ggplot(speciescountdate %>% filter(Species=="Adults total"& Location!="Sorghum"))+
  ...
  theme(text = element_text(size=15)) +
  theme(axis.text.x=element_text(angle=40, vjust=1, hjust=1))
ggplot(speciescountdate %>% filter(Species=="Adults total"), aes(x=Date, y=Count, color=Treatment))+
  geom_jitter(size=2)
...
ggplot(feedinginjurydata)+
  geom_jitter(aes(x=Date, y=Probes, color=Treatment))
ggplot(feedinginjurydata, aes(x=Date, y=Probes, fill=Date))+
  geom_violin(alpha=.6)+
  geom_jitter(size=.4, alpha=.9)+
  theme_ipsum()+
...
lmerfeed<-lmer(Probes~Date+Treatment+(1|Row/Section), data=feedinginjurydata)
Anova(lmerfeed)
emfeed<-emmeans(lmerfeed,~Treatment:Date)
emfeed
plot(emfeed)
